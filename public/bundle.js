...
const contenedorCategoria$1 = document.getElementById("categorias");
...
categorias.forEach((categoria) => {
  ...
  const plantilla = `
    ...
    contenedorCategoria$1.append(nuevaCategoria);
});
...
const cargarAnteriorSiguiente = (direccion) => {
  ...
  let indexImagenActaul;
  fotos.forEach((foto, index) => {
    if (foto.id === idImagenActual) {
      indexImagenActaul = index;
    }
  });
  if (direccion === "siguiente") {
    if (fotos[indexImagenActaul + 1]) {
      const { id, nombre, ruta, descripcion } = fotos[indexImagenActaul + 1];
